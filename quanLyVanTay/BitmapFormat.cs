...
namespace HW_Lib_Test
{
    public class BitmapFormat
    {
        ...
        * 函数返回：无
        *********************************************/
        public static void WriteBitmap(byte[] buffer, int nWidth, int nHeight,string filename)
        {
            int ColorIndex = 0;
            ushort m_nBitCount = 8;
            int m_nColorTableEntries = 256;
            ...
            try
            {
                ...
                Stream FileStream = File.Open(Environment.GetFolderPath(Environment.SpecialFolder.MyPictures)+"\\"+filename+".bmp", FileMode.Create, FileAccess.Write);
